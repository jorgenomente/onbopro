...
last_pass as (
  select
    ...
    qa.quiz_id,
    max(qa.created_at) filter (where qa.passed = true) as last_passed_at
  ...
  select
    ...
    count(*) filter (
      where qa.passed = false
        and qa.created_at > coalesce(lp.last_passed_at, '-infinity'::timestamptz)
    )::int as consecutive_failed_count
  from public.quiz_attempts qa
  join org_access oa on oa.org_id = qa.org_id
  left join last_pass lp
    on lp.org_id = qa.org_id
   and lp.learner_id = qa.user_id
   and lp.quiz_id = qa.quiz_id
  where qa.submitted_at is not null
  group by qa.org_id, qa.user_id, qa.quiz_id, lp.last_passed_at
),
quiz_failure_max as (
  select
    qc.org_id,
    qc.learner_id,
    max(qc.consecutive_failed_count)::int as max_failed_count
  from quiz_consecutive qc
  where qc.consecutive_failed_count >= 3
  group by qc.org_id, qc.learner_id
),
alert_base as (
  select
    lb.org_id,
...
select
  ...
  case ab.alert_type
    when 'quiz_failed'
      then 'Falló el mismo quiz 3 o más veces desde su último aprobado.'
