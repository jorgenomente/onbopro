...
begin
  ...
  create temporary table _template_lesson_map (
    template_lesson_id uuid primary key,
  ...
  loop
    ...
    returning id into v_course_lesson_id;

    insert into _template_lesson_map (template_lesson_id, course_lesson_id)
    values (v_lesson.lesson_id, v_course_lesson_id);
  end loop;
  ...
  join _template_lesson_map lm
    on lm.template_lesson_id = b.lesson_id
