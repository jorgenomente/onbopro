...
-- selected questions (attempt set if exists, otherwise full bank)
selected_questions as (
  ...
  join attempt_pick ap
    on ap.local_id = qa.local_id
   and ap.quiz_id = qa.quiz_id
  join quiz_attempt_questions aq
    on aq.attempt_id = ap.attempt_id
...
questions_json as (
  select
    ...
    jsonb_agg(
      jsonb_build_object(
        ...
        'prompt', qb.prompt,
        'explanation', case
          when ap.submitted_at is not null and qa.show_correct_answers
            then qb.explanation
          else null
        end,
        'options', (
  ...
  left join selected_questions sq
    on sq.local_id = qa.local_id
   and sq.quiz_id = qa.quiz_id
  left join questions_base qb on qb.question_id = sq.question_id
...
select
  ...
  qa.time_limit_min,
  qa.pass_score_pct,
  qa.shuffle_questions,
  qa.show_correct_answers,
