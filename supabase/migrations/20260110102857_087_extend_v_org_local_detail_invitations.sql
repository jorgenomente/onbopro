  ...
  select
    ...
    case
      when coalesce(tl.total_lessons, 0) = 0 then null
        ...
        / nullif(coalesce(tl.total_lessons, 0), 0)
      )::numeric
    end as progress_pct,
    case
...
invitations_json as (
  ...
  join locals_base lb on lb.local_id = i.local_id
  group by i.local_id
)
select
  ls.org_id,
...
left join invitations_json ij on ij.local_id = ls.local_id;

-- Sanity checks (manual)
-- select * from public.v_org_local_detail where local_id = '<local_id>';
