...
local_metrics as (
  select
    ...
    (coalesce(ac.active_courses_count, 0) > 0 and coalesce(lc2.total_lessons, 0) > 0)
      as has_assignments,
    (la.last_activity_at is not null) as has_any_activity
  from locals_base lb
...
local_status as (
  select
    ...
    case
      ...
      when not lm.has_assignments then 'on_track'
      when not lm.has_any_activity then 'on_track'
    ...
    case
      ...
      when not lm.has_assignments then null
      when not lm.has_any_activity then 'not_started'
